...
void ServerStorageMemory::DumpData(const std::string &dname)
{
    ...
    for (const auto &pair : storage_)
    {
        ...
        if (l_size > 0) file.write(cell.L.data(), l_size);
        if (d_size > 0) file.write(cell.D.data(), d_size);
        if (c_size > 0) file.write(cell.C.data(), c_size);
    }
    file.close();
}
...
void ServerStorageMemory::LoadData(const std::string &dname)
{
    ...
    while (file)
    {
        ...
        if (l_size > 0) file.read(cell.L.data(), l_size);
        if (d_size > 0) file.read(cell.D.data(), d_size);
        if (c_size > 0) file.read(cell.C.data(), c_size);
