...
ServerStorageSQL::ServerStorageSQL() : conn{"postgresql://sse:123456@127.0.0.1/bamboo"}
{
...
bool ServerStorageSQL::Get(EDBCell &in_out)
{
    ...
    const unsigned char *l_data = reinterpret_cast<const unsigned char *>(in_out.L.data());
...
void ServerStorageSQL::Put(const EDBCell &in)
{
    ...
    pqxx::binarystring L_bin{reinterpret_cast<const unsigned char *>(in.L.data()), in.L.size()};
    pqxx::binarystring D_bin{reinterpret_cast<const unsigned char *>(in.D.data()), in.D.size()};
    pqxx::binarystring C_bin{reinterpret_cast<const unsigned char *>(in.C.data()), in.C.size()};
...
void ServerStorageSQL::PushBatch(const std::vector<EDBCell> &cip_all)
{
    ...
    try
    {
        ...
        {
            pqxx::binarystring L_bin{reinterpret_cast<const unsigned char *>(itr.L.data()), itr.L.size()};
            pqxx::binarystring D_bin{reinterpret_cast<const unsigned char *>(itr.D.data()), itr.D.size()};
            pqxx::binarystring C_bin{reinterpret_cast<const unsigned char *>(itr.C.data()), itr.C.size()};
            w.exec_prepared0("put_data", L_bin, D_bin, C_bin);
        }
        w.commit();
    }
