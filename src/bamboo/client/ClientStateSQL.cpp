...
bool ClientStateSQL::Get(StateCell &out, const string &keyword) {
  ...
  sqlite3_prepare_v2(db, "SELECT cnt, tk FROM State WHERE keyword=?1;", -1,
                     &stmt_read_data, nullptr);
...
void ClientStateSQL::Put(const StateCell &in, const string &keyword) {
  ...
  sqlite3_prepare_v2(
      db, "REPLACE INTO State (keyword, cnt, tk) VALUES (?1, ?2, ?3);", -1,
      &stmt_update_data, nullptr);
...
void ClientStateSQL::Clear() {
  ...
  sqlite3_prepare_v2(db, "DELETE FROM State;", -1, &stmt_clear_data, nullptr);
...
void ClientStateSQL::GetKeywordsCnt(vector<int> &cnt) {
  ...
  sqlite3_prepare_v2(db, "SELECT cnt FROM State;", -1, &stmt_get_cnt, nullptr);
