...
void BambooServer::Setup() { srv_store->Clear(); }

void BambooServer::Save(const std::string &L, const std::string &D,
                        const std::string &C) {
  EDBCell cell = {.L = L, .D = D, .C = C};

  srv_store->Put(cell);
}

void BambooServer::Search(std::vector<std::string> &result,
                          const std::string &K_in, const std::string &L_in,
                          const std::string &MskD_in,
                          const std::string &MskC_in) {
...
void BambooServer::KeyUpdate_Parallel(const string &token, int num_threads) {
  ...
  _num_threads =
      num_threads > (int)cells.size() ? (int)cells.size() : num_threads;

  for (int i = 0; i < _num_threads; i++) {
    threads[i] = std::thread(do_KeyUpdate_in_parallel, std::ref(cells), i,
                             std::ref(x), _num_threads);
  }
...
void BambooServer::SaveBatch(const vector<std::string> &Ls,
                             const vector<std::string> &Ds,
                             const vector<std::string> &Cs) {
  vector<EDBCell> cells;
...
void do_KeyUpdate_in_parallel(vector<EDBCell> &cells, int number, string &delta,
                              int num_threads) {
  bn_t d;
  ep_t ele1;
