...
void test_client() {
  ...
  for (int i = 0; i < 200; i++) {
    sse_client.DataUpdate("abc", "file-" + std::to_string(i), Poseidon_add);
  }
  cout << "Encrypted " << 200 << " ciphers for abc" << endl;

  for (int i = 0; i < 100; i++) {
    sse_client.DataUpdate("def", "file-" + std::to_string(i), Poseidon_add);
  }
  cout << "Encrypted " << 100 << " ciphers for def" << endl;
  result.clear();
  sse_client.Search(result, {"abc", "def"});
  for (auto &id : result) {
    cout << id << endl;
  }
  ...
  //   sse_client.DataUpdate("def", "file-" + std::to_string(i), Poseidon_del);
  // }
  ...
  //   sse_client.DataUpdate("abc", "file-" + std::to_string(i), Poseidon_del);
  // }
