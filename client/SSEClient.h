...
public:
    ...
    void prepare_dataset(const std::map<std::string, std::vector<std::string>> &data_to_encrypt, BambooOp op = Bamboo_add);

    void Setup();

    void DataUpdate(const std::string &keyword, const std::string &id, BambooOp op);
